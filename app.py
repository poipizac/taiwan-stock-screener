            ...
            if csv_files:
                ...
                if not target_data.empty:
                        ...
                        try:
                            ...
                            if not t_row.empty:
                                history_records.append({
                                    'date': date_obj.strftime('%Y-%m-%d'), # 與下方對齊格式
                                    'Foreign': t_row.iloc[0]['外資買賣超'],
                                    'Trust': t_row.iloc[0]['投信買賣超']
                                })
                        except Exception:
                            pass # 解析失敗的檔案安全跳過
                    ...
                    if history_records:
                        ...
                        # 💡 神奇魔法：將這些歷史資料偽裝成 FinMind 的資料結構，交給後面的程式碼處理！
                        foreign_df = history_df[['Foreign']].copy()
                        trust_df = history_df[['Trust']].copy()
                    # ==================================
